/***************************************************************************************
*
*    Title: input-knobs.js
*    Author: g200kg
*    Availability: https://g200kg.github.io/input-knobs/
*
***************************************************************************************/
...
<circle fill="${fg}" cx="${w * 0.5}" cy="${h * 1.5}" r="${minwh * 0.25}"/></g></svg>`
  ...
  const initKnobs = (el) => {
    ...
    el.refresh = () => {
      ...
      if (src) {
        ...
        if (ik.sprites >= 1) { el.style.backgroundSize = `100% ${(ik.sprites + 1) * 100}%` } else if (ik.itype != 'k') {
    ...
    el.redraw = (f) => {
      if (f || ik.valueold != el.value) {
        ...
        if (ik.sprites >= 1) {
          let difFaktor = 14
          if (Array.from(el.parentElement.classList).includes('plug-adjust')) {
            difFaktor = 22
          }
  ...
  addEventListener('resize', (event) => {
    const elem = document.querySelectorAll('input.input-knob,input.input-slider')
    for (let i = 0; i < elem.length; ++i) { elem[i].redraw(true) }
  })
})
